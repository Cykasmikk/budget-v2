...
from src.infrastructure.repository import SQLBudgetRepository, SQLRuleRepository
...
async def get_upload_use_case(session: AsyncSession = Depends(get_session)):
    ...
    analyzer = AnalyzeBudgetUseCase(repo, rule_repo)
...
async def get_analyze_use_case(session: AsyncSession = Depends(get_session)):
    repo = SQLBudgetRepository(session)
    rule_repo = SQLRuleRepository(session)
    return AnalyzeBudgetUseCase(repo, rule_repo)
