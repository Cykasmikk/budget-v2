...
class AIChatService:
    ...
    def _extract_merchant(self, description: str) -> str:
        """
        Extract merchant name from transaction description using the shared validaton service.
        """
        return MerchantNormalizationService.normalize(description)
    ...
    async def generate_response(self, message: str, conversation_history: List[Dict[str, str]] = []) -> tuple[str, str]:
        ...
        if self.llm_provider:
            ...
            # Ensure safe serialization (handle Decimals if present)
            ...
            if self.verifier:
                ...
                full_explanation = f"Generated by Google Gemini Flash 2.0.\nContext: Analyzed {context.entry_count} transactions (${context.total_expenses:,.2f}).\nTrace: {verification_details}"
            else:
                earliest = context.date_range.earliest[:10] if context.date_range and context.date_range.earliest else "N/A"
                latest = context.date_range.latest[:10] if context.date_range and context.date_range.latest else "N/A"
                
                full_explanation = (
                    f"**AI Generation Metadata**\n"
                    f"- **Model:** Google Gemini Flash 2.0\n"
                    f"- **Context:** {context.entry_count} transactions analyzed\n"
                    f"- **Total Spend:** ${context.total_expenses:,.2f}\n"
                    f"- **Date Range:** {earliest} to {latest}\n"
                    f"- **Entities:** {len(context.categories)} Categories, {len(context.all_merchants)} Merchants"
                )
    ...
    async def generate_response_stream_with_meta(self, message: str, conversation_history: List[Dict[str, str]] = []) -> AsyncIterator[Dict[str, str]]:
        ...
        try:
            ...
            if self.llm_provider:
                 ...
                 logger.info("ai_chat_llm_response_received", length=len(full_response))
            ...
            # --- Fallback Heuristic Logic (simulated streaming) ---
            logger.info("ai_chat_fallback_logic")
            message_lower = message.lower()
            ...
            logger.error("ai_chat_stream_failed", error=str(e), traceback=True)
            yield {'type': 'token', 'content': f"Error generating response: {str(e)}"}