  ...
  `;
    ...
    private createForecastConfig(data: ChartData, isMultiLine: boolean, computedStyle: CSSStyleDeclaration): ChartConfig {
        ...
        if (isMultiLine) {
            ...
            labels = trendData.map((d: { month: string; amount: number; is_forecast: boolean }) => d.month);
            const values = trendData.map((d: { month: string; amount: number; is_forecast: boolean }) => d.amount);
                    ...
                    tooltip: {
                        callbacks: {
                            label: (context: any) => {
                                ...
                                return `${context.dataset?.label || 'Value'}: $${val?.toLocaleString() ?? '0'}`;
                            }
                        }
                    }
                },
