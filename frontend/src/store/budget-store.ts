...
import { QueryResult, BreakdownDict, MerchantMap, HistoryData, HistoryItem, ForecastSummary, TimelineItem, ViewMode, Rule } from '../types/interfaces';
...
export class BudgetStore {
    ...
    setData(data: { transactions: Transaction[], metrics: BudgetMetrics }, fileName?: string, fileSize?: string) {
        ...
        // Demo Logic: Enrich transactions if missing neuro-symbolic fields
        data.transactions = (data.transactions || []).map(t => ({
            ...t,
            amount: Number(t.amount),
            confidence_score: t.confidence_score ?? (t.category === 'Uncategorized' ? 0.6 : 0.9 + Math.random() * 0.1),
            is_rule_match: t.is_rule_match ?? (['Rent', 'Spotify', 'AWS'].some(k => t.description.includes(k)))
        }));
        ...
        // Also update the global transaction list with our enriched data
        this.store.setState(state => ({
            ...state,
            transactions: data.transactions || []
        }));
    ...
    loadSampleData(variant?: string) {
        ...
        try {
            ...
            // Generate Mock Transactions to populate the Budget Table
            const mockTransactions: Transaction[] = [];
            const merchantList = Object.keys(data.top_merchants || {});
            ...
            for (let i = 0; i < 50; i++) {
                ...
                mockTransactions.push({
                    ...
                    description: `${merchant} - Invoice #${1000 + i}`,
                    confidence_score: 0.8 + (Math.random() * 0.19), // 0.80 - 0.99
                    is_rule_match: Math.random() > 0.5,
                    type: 'OpEx'
                });
            }
            ...
            const newState = {
                ...
                transactions: mockTransactions, // Populate transactions
